...
export default function RouteFilterDialog({ item, open, onDismiss, onConfirm }) {
    ...
    const messageText = item.comment
        ? `You're editing a route filter for all OSPF routes.`
        : `Dynamic routes can't have comments, so this action creates a route filter which matches against all OSPF routes.`;
