...
module.exports = async ({ address, uiPort }) => {
    const response = await turtleWebApi.get("cgi-bin/getjson.cgi?json=dante", {
        address,
    });
    ...
    for (let eachDevice of devices) {
        ...
        await devicesCollection.replaceOne({ name: eachDevice.name }, parsedDevice, {
            upsert: true,
        });

        // get source labels
        const sources = eachDevice.txchn
        ...
        const destinations = eachDevice.rxchn
            .filter((c) => c.type === "audio")
            .map((c) => {
                return {
                    name: c.name,
        ...
        // and finally, routes
        const routes = eachDevice.rxchn
            .filter((rxc) => rxc.status === "DYNAMIC")
            .map((c) => {
    ...
    await devicesCollection.updateMany({ timestamp: { $lt: thirtySecondsAgo } }, { $set: { active: false } });
