    ...
    while (true) {
        ...
        if (!interfaces) {
            ...
            const ifAutoNegotiation = await snmpAwait.subtree({
                maxRepetitions: 1000,
                oid: "1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.20",
            });

            const ifAdminPortSpeed = await snmpAwait.subtree({
                maxRepetitions: 1000,
                oid: "1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.5",
            });

            const ifOperationalPortSpeed = await snmpAwait.subtree({
                maxRepetitions: 1000,
                oid: "1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.25",
            });

            for (let eachInterface of interfaces) {
                const alias = ifAliases[`1.3.6.1.2.1.31.1.1.1.18.${eachInterface.interfaceId}`];
                const autoNegotiationState =
                    ifAutoNegotiation[`1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.20.${eachInterface.interfaceId}`] === 1;
                const adminPortSpeed = convertAdminPortSpeed(
                    ifAdminPortSpeed[`1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.5.${eachInterface.interfaceId}`]
                );
                const operationalPortSpeed = convertOperationalPortSpeed(
                    ifOperationalPortSpeed[`1.3.6.1.4.1.52642.1.1.10.2.10.1.1.1.25.${eachInterface.interfaceId}`]
                );
