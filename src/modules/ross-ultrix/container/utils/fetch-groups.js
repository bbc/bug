...
module.exports = async ({ address, uiPort }) => {
    ...
    const response = await ultrixWebApi.get("groupcategory/agdatassds", {
        address,
        uiPort,
    });
    ...
    if (response?.[0]?.Name === "GroupCategories") {
        ...
        groups = response?.[0]?.Children.filter((c) => c.Type === 1).map((n) => {
            return {
                id: n.Id,
                name: n.Name,
                sources: n.Children?.filter((c) => c.Type === 3).map(
                    (s) => sourceNames.find((sn) => sn.name === s.Name)?.uiId
                ),
                destinations: n.Children?.filter((c) => c.Type === 2).map(
                    (d) => destinationNames.find((dn) => dn.name === d.Name)?.uiId
                ),
            };
        });
    }
