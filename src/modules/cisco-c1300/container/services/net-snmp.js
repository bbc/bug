...
function _expandConstantObject(object) {
    ...
    for (var i = 0; i < keys.length; i++) object[object[keys[i]]] = parseInt(keys[i]);
}
        ...
        if (varbinds[i].type && varbinds[i].hasOwnProperty("value")) {
            ...
            switch (type) {
                ...
                case ObjectType.IpAddress:
                    ...
                    if (bytes.length != 4) throw new RequestInvalidError("Invalid IP address '" + value + "'");
...
TrapPdu.prototype.toBuffer = function (buffer) {
    ...
    buffer.writeBuffer(Buffer.from(this.agentAddr.split(".")), ObjectType.IpAddress);
...
Authentication.isAuthentic = function (messageBuffer, authProtocol, authPassword, engineID, digestInMessage) {
    ...
    if (digestInMessage.length !== Authentication.algorithms[authProtocol].AUTHENTICATION_CODE_LENGTH) return false;
...
Message.createFromBuffer = function (buffer, user) {
    ...
    if (message.version != 3) {
        ...
        message.msgSecurityParameters.msgAuthoritativeEngineID = msgSecurityParametersReader.readString(
            ber.OctetString,
            true
        );
...
var Session = function (target, authenticator, options) {
    ...
    this.reportOidMismatchErrors =
        typeof options.reportOidMismatchErrors !== "undefined" ? options.reportOidMismatchErrors : false;
    ...
    if (message.version != req.message.version) {
        ...
        if (this.proxy) {
            ...
            if (!req.originalPdu || !req.allowReport) {
                if (
                    Array.isArray(message.pdu.varbinds) &&
                    message.pdu.varbinds[0] &&
                    message.pdu.varbinds[0].oid.indexOf(UsmStatsBase) === 0
                ) {
                    this.userSecurityModelError(req, message.pdu.varbinds[0].oid);
                    return;
                }
                req.responseCb(
                    new ResponseInvalidError("Unexpected Report PDU", ResponseInvalidCode.EUnexpectedReport)
                );
                return;
            }
        ...
        req.responseCb(
            new ResponseInvalidError(
                "Unknown PDU type '" + message.pdu.type + "' in response",
                ResponseInvalidCode.EUnknownPduType
            )
        );
    }
...
function subtreeCb(req, varbinds) {
    ...
    if (varbinds.length > 0) req.feedCb(varbinds);
...
function walkCb(req, error, varbinds) {
    ...
    if (this.version == Version2c || this.version == Version3) {
        ...
        if (req.feedCb(varbinds[0])) done = 1;
        if (!done) oid = varbinds[0][varbinds[0].length - 1].oid;
    } else {
        if (!done) {
    ...
    if (done) req.doneCb(null);
    else this.walk(oid, req.maxRepetitions, req.feedCb, req.doneCb, req.baseOid);
}
...
Session.prototype.walk = function () {
    ...
    else this.getNext([oid], walkCb.bind(me, req));
...
Listener.processIncoming = function (buffer, authorizer, callback) {
    ...
    if (message.version == Version3) {
        message.user = authorizer.users.filter(
            (localUser) => localUser.name == message.msgSecurityParameters.msgUserName
        )[0];
            ...
            if (message.msgSecurityParameters.msgUserName != "" && !authorizer.disableAuthorization) {
                callback(
                    new RequestFailedError(
                        "Local user not found for message with user " + message.msgSecurityParameters.msgUserName
                    )
                );
                return;
        ...
        if (
            (message.user.level == SecurityLevel.authNoPriv || message.user.level == SecurityLevel.authPriv) &&
        ...
        if (!community && !authorizer.disableAuthorization) {
            callback(
                new RequestFailedError("Local community not found for message with community " + message.community)
            );
            return;
        }
    }
...
Receiver.prototype.onMsg = function (buffer, rinfo) {
    ...
    if (message.pdu.type == PduType.Trap || message.pdu.type == PduType.TrapV2) {
        ...
        this.callback(
            new RequestInvalidError("Unexpected PDU type " + message.pdu.type + " (" + PduType[message.pdu.type] + ")")
        );
    }
};
...
Mib.convertOidToAddress = function (oid) {
    ...
    if (address.length < 1) throw new RangeError("object identifier is too short");
    ...
    for (let i = 0; i < requestPdu.varbinds.length; i++) {
        ...
        if (!instanceNode) {
            ...
            if (!providerNode || instanceNode.value === undefined) {
                ...
                this.authorizer.getAccessControlModelType() == AccessControlModelType.Simple &&
                !this.authorizer
                    .getAccessControlModel()
                    .isAccessAllowed(requestMessage.version, securityName, requestMessage.pdu.type)
            ) {
                ...
                requestPdu.type === PduType.SetRequest &&
                ...
                requestPdu.varbinds[i].requestValue = this.castSetValue(
                    requestPdu.varbinds[i].type,
                    requestPdu.varbinds[i].value
                );
            ...
            mibRequests[savedIndex].done = function (error) {
                ...
                if (error) {
                    ...
                    if (requestPdu.type == PduType.SetRequest) {
                        ...
                        if (
                            ...
                            let setResult = mibRequests[savedIndex].instanceNode.setValue(
                                me.castSetValue(
                    ...
                    if (
                        (requestPdu.type == PduType.GetNextRequest || requestPdu.type == PduType.GetBulkRequest) &&
                ...
                if (requestPdu.type == PduType.SetRequest) {
                    ...
                    if (requestPdu.varbinds[savedIndex].requestValue) {
                        responseVarbind.requestValue = me.castSetValue(
                            requestPdu.varbinds[savedIndex].type,
                            requestPdu.varbinds[savedIndex].requestValue
                        );
                    } else {
                        responseVarbind.requestValue = me.castSetValue(
                            requestPdu.varbinds[savedIndex].type,
    ...
    if (varbind.type && varbind.oid) {
        switch (varbind.type) {
            ...
            case ObjectType.IpAddress:
                ...
                if (bytes.length != 4) throw new RequestInvalidError("Invalid IP address '" + varbind.value + "'");
