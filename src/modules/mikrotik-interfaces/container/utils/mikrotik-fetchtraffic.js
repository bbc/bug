...
module.exports = async (routerOsApi, interfaceName) => {
    ...
    try {
        const data = await routerOsApi.run("/interface/monitor-traffic", ["=interface=" + interfaceName, "=once="]);
        ...
        logger.error(`error fetching interface information for ${interfaceName}`);
    }
    return null;
};
