...
const fetch = require("./fetch-with-timeout");
...
class DigestClient {
    ...
    async fetch(url, options = {}, timeout = 2000) {
        if (this.basic) return fetch(url, this.addBasicAuth(options), timeout);
        const resp = await fetch(url, this.addAuth(url, options), timeout);
        if (resp.status == 401 || (resp.status == this.statusCode && this.statusCode)) {
            this.hasAuth = false;
            await this.parseAuth(resp.headers.get("www-authenticate"));
            if (this.hasAuth) {
                const respFinal = await fetch(url, this.addAuth(url, options), timeout);
