...
module.exports = async (uuid) => {
    ...
    if (!vms) {
        throw new Error(`vm-disableautopower: no VMs found in database`);
    }

    // get the vm details from the db (we need it for the ref too...)
    const vm = vms.find((item) => item.uuid === uuid);
    if (!vm) {
        throw new Error(`vm-disableautopower: could not find VM with uuid ${uuid}`);
    }

    // let's overwrite the db field - it'll be replaced once the worker detects the real state
    vm.other_config.auto_poweron = "false";

    // saving to db
    mongoSingle.set("vms", vms, 30);

    await xapi.connect();
    console.log(`vm-disableautopower: rebooting vm uuid ${uuid}`);
    await xapi.call("VM.set_other_config", vm.ref, { auto_poweron: "false" });
    return true;
};
