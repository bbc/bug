...
export default function InterfaceList({ panelId, stackId = null }) {
    ...
    const handleDetailsClicked = (event, item) => {
        navigate(`/panel/${panelId}/interface/${encodeURIComponent(item.interfaceId)}`);
    };

    const handleNeighborLinkClicked = (event, item) => {
        event.stopPropagation();
        navigate(`/panel/${panelId}/interface/${encodeURIComponent(item.interfaceId)}/neighbor`);
    };

    const handleDevicesLinkClicked = (event, item) => {
        event.stopPropagation();
        navigate(`/panel/${panelId}/interface/${encodeURIComponent(item.interfaceId)}/devices`);
    };
        ...
        <BugApiTable
            columns={[
                ...
                },
                {
                    ...
                    width: 70,
                    content: (item) => {
                        if (item["admin-status"] === undefined) {
                            return <BugApiSwitch disabled={true} />;
                        }
                        return (
                            <BugApiSwitch
                                checked={item["admin-status"] === "if-state-up"}
